...
# 必须添加这个样式定义（关键！）
style crystal_button_text is button_text
style crystal_button is button
style crystal_button_text:
    color "#E0E0E0"
    hover_color "#76B900"
style crystal_button:
    hover_background "#2D2D2D"


screen crystal_choice():
    modal True  # 阻止点击其他区域
    
    # 居中显示水晶球
    add "crystal_ball" at truecenter:
        zoom(0.4)
    ...
    vbox:
        ...
        textbutton "追寻未来的光" action [
            SetVariable("chosen_route", "future"),
            Show("crystal_activate", transition=None),
            Jump("htt_sight")
        ] style "crystal_button"
        
        textbutton "沉入过去的影" action [
            SetVariable("chosen_route", "past"),
            Show("crystal_activate", transition=None),
            Jump("lyt_sight")
        ] style "crystal_button"

# 激活时的发光演出层
screen crystal_activate():
    layer "master"
    zorder 100
    
    # 播放发光动画 + 闪光
    add "crystal_ball_glow" at truecenter:
        zoom(2.0)
    timer 0.8 action Hide("crystal_activate", transition=flash)
