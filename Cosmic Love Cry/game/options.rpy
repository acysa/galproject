...
define config.name = _("Cosmic Love Cry")
...
define build.name = "CosmicLoveCry"
...
define config.save_directory = "CosmicLoveCry-1782313199"
...
define config.window_icon = "gui/bf3_2.png"
...
init python:
    def show_random_char(base_name, expressions, pos="left", flip=False, yoffset=0, zoom=1.0):
        choice = renpy.random.choice(expressions)
        full_image = f"{base_name} {choice}"
        
        # 1. 获取基础位置的坐标 (Ren'Py 内置的 left/right/center 都有 xpos 属性)
        if pos == "right":
            base_pos = right
        elif pos == "center":
            base_pos = center
        ...
        # 2. 计算最终的 X 轴缩放（如果翻转，xzoom 为负数）
        final_xzoom = -1.0 if flip else 1.0
        
        # 3. 创建一个纯粹的变换，只包含缩放和偏移（不包含位置）
        if flip or yoffset != 0 or zoom != 1.0:
            extra_transform = Transform(xzoom=final_xzoom, yoffset=yoffset,zoom=zoom)
            # 把【位置】和【变换】分开，放进列表里传给引擎
            final_transforms = [base_pos, extra_transform]
        else:
            final_transforms = [base_pos]
            
        # 4. 显示图片并加上溶解效果
        renpy.show(full_image, at_list=final_transforms)
